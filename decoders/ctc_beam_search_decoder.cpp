...
std::vector <std::pair<int, int>> BeamDecoder::get_word_timestamps_python() {
    std::vector <std::pair<int, int>> output;
    for (std::tuple <std::string, int, int> w: wordlist) {
        std::pair<int, int> word_ts;
        word_ts.first = std::get<1>(w);
        word_ts.second = std::get<2>(w);
        output.emplace_back(word_ts);
    }
    return output;
}
...
std::vector <std::future<std::vector < std::pair < double, std::string>>>>
res;
for (
size_t i = 0;
i<batch_size;
...
emplace_back(pool
.
enqueue(ctc_beam_search_decoder,
        probs_split[i],
        vocabulary,
        beam_size,
        cutoff_prob,
